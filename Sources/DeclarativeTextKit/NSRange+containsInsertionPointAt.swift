...
extension NSRange {
    /// Unlike regular `contains(_:)`, also returns `true` *at* the end position, which is not a valid position to read from, but a valid location to append content to.
    ///
    /// - Returns: `true` iff `location` is between or equal `lowerBound` or `upperBound`.
    @inlinable @inline(__always)
    public func contains(insertionPointAt location: Int) -> Bool {
